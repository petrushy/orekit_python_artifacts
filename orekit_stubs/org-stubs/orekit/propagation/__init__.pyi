...
class AdditionalDataProvider(typing.Generic[_AdditionalDataProvider__T]):
    ...
    public interface AdditionalDataProvider<T>
        ...
        This interface allows to modify :class:`~org.orekit.propagation.SpacecraftState` and set up additional data.
        ...
        :meth:`~org.orekit.propagation.SpacecraftState.addAdditionalData`. Instances of classes implementing this interface are
        intended to be registered to propagators so they can either modify the basic components (orbit, attitude and mass) or
        add additional data incrementally after having computed the basic components.
    
        Some additional data may depend on previous additional data to be already available the before they can be computed. It
        may even be impossible to compute some of these additional data at some time if they depend on conditions that are
        ...
        :meth:`~org.orekit.propagation.AdditionalDataProvider.update` methods in an order that fulfill these dependencies that
        ...
        *not* :meth:`~org.orekit.propagation.AdditionalDataProvider.yields` when asked. Consider for example a case where
        providers A, B and C have been registered and provider B needs in fact the additional data generated by provider C. Then
        when a complete state is built, the propagator puts the three providers in a new stack, and then starts the incremental
        generation of additional data. It first checks provider A which does not yield so it is popped from the stack and the
        additional data it generates is added. Then provider B is checked, but it yields because state from provider C is not
        yet available. So propagator checks provider C which does not yield, so it is popped out of the stack and applied. At
        this stage, provider B is the only remaining one in the stack, so it is checked again, but this time it does not yield
        because the state from provider C is available as it has just been added, so provider B is popped from the stack and
        applied. The stack is now empty and the propagator can return the completed state.
    
        It is possible that at some stages in the propagation, a subset of the providers registered to a propagator all yield
        and cannot :meth:`~org.orekit.propagation.AdditionalDataProvider.update` the data. This happens for example during the
        ...
        state will all yield. This behavior is expected. Another case occurs when users set up additional data that induce a
        dependency loop (data A depending on data B which depends on data C which depends on data A). In this case, the three
        corresponding providers will wait for each other and indefinitely yield. This second case is a deadlock and results from
        a design error of the additional data management at application level. The propagator cannot know it in advance if a
        subset of providers that all yield is normal or not. So at propagator level, when either situation is detected, the
        propagator just gives up and returns the most complete state it was able to compute, without generating any error.
        Errors will indeed not be triggered in the first case (once the primary equations have been initialized, the secondary
        equations will be initialized too), and they will be triggered in the second case as soon as user attempts to retrieve
        an additional data that was not added.
    ...
    """
    def getAdditionalData(self, spacecraftState: 'SpacecraftState') -> _AdditionalDataProvider__T:
        """
    ...
    def init(self, spacecraftState: 'SpacecraftState', absoluteDate: org.orekit.time.AbsoluteDate) -> None:
        """
            ...
            Parameters:
                initialState (:class:`~org.orekit.propagation.SpacecraftState`): initial spacecraft state information at the start of propagation
...
class CartesianToleranceProvider:
    ...
    public interface CartesianToleranceProvider
        ...
        Interface to define integration tolerances for adaptive schemes (like the embedded Runge-Kutta ones) propagating the
        position-velocity vector and the mass, for a total of 7 primary dependent variables (in that order). The tolerances are
        given as an array of array: each row has 7 elements, whilst the first column is the absolute tolerances and the second
        the relative ones.
        ...
        Also see:
            ...
            :class:`~org.orekit.propagation.CartesianToleranceProvider`
    """
    ...
    def getTolerances(self, vector3D: org.hipparchus.geometry.euclidean.threed.Vector3D, vector3D2: org.hipparchus.geometry.euclidean.threed.Vector3D) -> typing.MutableSequence[typing.MutableSequence[float]]:
        ...
        ...
    @typing.overload
    def getTolerances(self, fieldVector3D: org.hipparchus.geometry.euclidean.threed.FieldVector3D[_getTolerances_1__T], fieldVector3D2: org.hipparchus.geometry.euclidean.threed.FieldVector3D[_getTolerances_1__T]) -> typing.MutableSequence[typing.MutableSequence[float]]:
        """
            Retrieve the integration tolerances given reference position and velocity Field vectors.
        
            Parameters:
                position (:class:`~org.orekit.propagation.https:.www.hipparchus.org.apidocs.org.hipparchus.geometry.euclidean.threed.FieldVector3D?is`<T> position): reference position vector
                velocity (:class:`~org.orekit.propagation.https:.www.hipparchus.org.apidocs.org.hipparchus.geometry.euclidean.threed.FieldVector3D?is`<T> velocity): reference velocity vector
    ...
    def getTolerances(self, cartesianOrbit: org.orekit.orbits.CartesianOrbit) -> typing.MutableSequence[typing.MutableSequence[float]]:
        """
            ...
            Parameters:
                cartesianOrbit (:class:`~org.orekit.orbits.CartesianOrbit`): reference orbit
            ...
            Parameters:
                absolutePVCoordinates (:class:`~org.orekit.utils.AbsolutePVCoordinates`): reference position-velocity
    ...
    def getTolerances(self, fieldCartesianOrbit: org.orekit.orbits.FieldCartesianOrbit[_getTolerances_3__T]) -> typing.MutableSequence[typing.MutableSequence[float]]:
        """
            ...
            Parameters:
                cartesianOrbit (:class:`~org.orekit.orbits.FieldCartesianOrbit`<T> cartesianOrbit): reference orbit
            ...
            Parameters:
                absolutePVCoordinates (:class:`~org.orekit.utils.FieldAbsolutePVCoordinates`<T> absolutePVCoordinates): reference position-velocity
...
class FieldAdditionalDataProvider(typing.Generic[_FieldAdditionalDataProvider__O, _FieldAdditionalDataProvider__T]):
    """
    public interface FieldAdditionalDataProvider<O, T extends :class:`~org.orekit.propagation.https:.www.hipparchus.org.apidocs.org.hipparchus.CalculusFieldElement?is`<T>>
    
        This interface allows to modify :class:`~org.orekit.propagation.FieldSpacecraftState` and set up additional data.
        ...
        :meth:`~org.orekit.propagation.FieldSpacecraftState.addAdditionalData` additional data}. Instances of classes
        implementing this interface are intended to be registered to propagators so they can either modify the basic components
        (orbit, attitude and mass) or add additional data incrementally after having computed the basic components.
    
        Some additional data may depend on previous additional data to be already available the before they can be computed. It
        may even be impossible to compute some of these additional data at some time if they depend on conditions that are
        fulfilled only after propagation as started or some event has occurred. As the propagator builds the complete state
        incrementally, looping over the registered providers, it must call their
        :meth:`~org.orekit.propagation.FieldAdditionalDataProvider.update` methods in an order that fulfill these dependencies
        ...
        *not* :meth:`~org.orekit.propagation.FieldAdditionalDataProvider.yields` when asked. Consider for example a case where
        providers A, B and C have been registered and provider B needs in fact the additional data generated by provider C. Then
        when a complete state is built, the propagator puts the three providers in a new stack, and then starts the incremental
        generation of additional data. It first checks provider A which does not yield so it is popped from the stack and the
        additional data it generates is added. Then provider B is checked, but it yields because data from provider C is not yet
        available. So propagator checks provider C which does not yield, so it is popped out of the stack and applied. At this
        stage, provider B is the only remaining one in the stack, so it is checked again, but this time it does not yield
        because the data from provider C is available as it has just been added, so provider B is popped from the stack and
        applied. The stack is now empty and the propagator can return the completed state.
    
        It is possible that at some stages in the propagation, a subset of the providers registered to a propagator all yield
        and cannot :meth:`~org.orekit.propagation.FieldAdditionalDataProvider.update` the data. This happens for example during
        the initialization phase of a propagator that computes State Transition Matrices or Jacobian matrices. These features
        are managed as secondary equations in the ODE integrator, and initialized after the primary equations (which correspond
        to orbit) have been initialized. So when the primary equation are initialized, the providers that depend on the
        secondary data will all yield. This behavior is expected. Another case occurs when users set up additional data that
        induce a dependency loop (data A depending on data B which depends on data C which depends on data A). In this case, the
        three corresponding providers will wait for each other and indefinitely yield. This second case is a deadlock and
        results from a design error of the additional data management at application level. The propagator cannot know it in
        ...
        retrieve an additional data that was not added.
    ...
    """
    def getAdditionalData(self, fieldSpacecraftState: 'FieldSpacecraftState'[_FieldAdditionalDataProvider__T]) -> _FieldAdditionalDataProvider__O: ...
    def getName(self) -> str:
        """
            Get the name of the additional data.
        ...
        """
        ...
    def init(self, fieldSpacecraftState: 'FieldSpacecraftState'[_FieldAdditionalDataProvider__T], fieldAbsoluteDate: org.orekit.time.FieldAbsoluteDate[_FieldAdditionalDataProvider__T]) -> None: ...
    def update(self, fieldSpacecraftState: 'FieldSpacecraftState'[_FieldAdditionalDataProvider__T]) -> 'FieldSpacecraftState'[_FieldAdditionalDataProvider__T]: ...
    def yields(self, fieldSpacecraftState: 'FieldSpacecraftState'[_FieldAdditionalDataProvider__T]) -> bool: ...
...
class FieldPropagator(org.orekit.utils.FieldPVCoordinatesProvider[_FieldPropagator__T], typing.Generic[_FieldPropagator__T]):
    ...
    """
    def addAdditionalDataProvider(self, fieldAdditionalDataProvider: FieldAdditionalDataProvider[typing.Any, _FieldPropagator__T]) -> None: ...
...
class FieldSpacecraftState(org.orekit.time.FieldTimeStamped[_FieldSpacecraftState__T], org.orekit.time.FieldTimeShiftable['FieldSpacecraftState'[_FieldSpacecraftState__T], _FieldSpacecraftState__T], typing.Generic[_FieldSpacecraftState__T]):
    ...
    def addAdditionalData(self, string: str, object: typing.Any) -> 'FieldSpacecraftState'[_FieldSpacecraftState__T]: ...
    ...
    def getAdditionalData(self, string: str) -> typing.Any:
        """
            ...
            Also see:
                :meth:`~org.orekit.propagation.FieldSpacecraftState.addAdditionalData`,
                :meth:`~org.orekit.propagation.FieldSpacecraftState.hasAdditionalData`,
                :meth:`~org.orekit.propagation.FieldSpacecraftState.getAdditionalDataValues`
        
        
        """
        ...
    def getAdditionalDataValues(self) -> org.orekit.utils.FieldDataDictionary[_FieldSpacecraftState__T]: ...
    def getAdditionalState(self, string: str) -> typing.MutableSequence[_FieldSpacecraftState__T]:
        """
        ...
        """
        ...
    def getAdditionalStateDerivative(self, string: str) -> typing.MutableSequence[_FieldSpacecraftState__T]:
        """
    ...
    def hasAdditionalData(self, string: str) -> bool:
        """
            ...
            Also see:
                :meth:`~org.orekit.propagation.FieldSpacecraftState.addAdditionalData`,
                :meth:`~org.orekit.propagation.FieldSpacecraftState.getAdditionalData`,
                :meth:`~org.orekit.propagation.FieldSpacecraftState.getAdditionalDataValues`
    ...
    def withAdditionalData(self, fieldDataDictionary: org.orekit.utils.FieldDataDictionary[_FieldSpacecraftState__T]) -> 'FieldSpacecraftState'[_FieldSpacecraftState__T]: ...
    def withAdditionalStatesDerivatives(self, fieldArrayDictionary: org.orekit.utils.FieldArrayDictionary[_FieldSpacecraftState__T]) -> 'FieldSpacecraftState'[_FieldSpacecraftState__T]: ...
    def withAttitude(self, fieldAttitude: org.orekit.attitudes.FieldAttitude[_FieldSpacecraftState__T]) -> 'FieldSpacecraftState'[_FieldSpacecraftState__T]: ...
    def withMass(self, t: _FieldSpacecraftState__T) -> 'FieldSpacecraftState'[_FieldSpacecraftState__T]: ...
...
class Propagator(org.orekit.utils.PVCoordinatesProvider):
    ...
    def setStepHandler(self, double: float, orekitFixedStepHandler: typing.Union[org.orekit.propagation.sampling.OrekitFixedStepHandler, typing.Callable]) -> None:
        ...
        ...
    @typing.overload
    def setStepHandler(self, orekitStepHandler: typing.Union[org.orekit.propagation.sampling.OrekitStepHandler, typing.Callable]) -> None:
        """
...
class SpacecraftState(org.orekit.time.TimeStamped, org.orekit.time.TimeShiftable['SpacecraftState']):
    """
    public class SpacecraftState extends :class:`~org.orekit.propagation.https:.docs.oracle.com.javase.8.docs.api.java.lang.Object?is` implements :class:`~org.orekit.time.TimeStamped`, :class:`~org.orekit.time.TimeShiftable`<:class:`~org.orekit.propagation.SpacecraftState`>
        ...
        additional data, which are simply named.
    ...
    def addAdditionalData(self, string: str, object: typing.Any) -> 'SpacecraftState':
        """
            ...
            instance, except it also has the specified state. If the original instance already had an additional data with the same
            ...
            Parameters:
                name (:class:`~org.orekit.propagation.https:.docs.oracle.com.javase.8.docs.api.java.lang.String?is`): name of the additional data (names containing "orekit" with any case are reserved for the library internal use)
                value (:class:`~org.orekit.propagation.https:.docs.oracle.com.javase.8.docs.api.java.lang.Object?is`): value of the additional data
            ...
            Also see:
                :meth:`~org.orekit.propagation.SpacecraftState.hasAdditionalData`,
                :meth:`~org.orekit.propagation.SpacecraftState.getAdditionalData`,
                :meth:`~org.orekit.propagation.SpacecraftState.getAdditionalDataValues`
    ...
    def getAdditionalData(self, string: str) -> typing.Any:
        """
            ...
            Also see:
                :meth:`~org.orekit.propagation.SpacecraftState.addAdditionalData`,
                :meth:`~org.orekit.propagation.SpacecraftState.hasAdditionalData`,
                :meth:`~org.orekit.propagation.SpacecraftState.getAdditionalDataValues`
    ...
    def getAdditionalState(self, string: str) -> typing.MutableSequence[float]:
        ...
        """
        ...
    def getAdditionalStateDerivative(self, string: str) -> typing.MutableSequence[float]:
        """
            Get an additional state derivative.
    ...
    def withAdditionalData(self, dataDictionary: org.orekit.utils.DataDictionary) -> 'SpacecraftState':
        """
            ...
            Parameters:
                dataDictionary (:class:`~org.orekit.utils.DataDictionary`): additional data
        ...
        """
        ...
    def withAdditionalStatesDerivatives(self, doubleArrayDictionary: org.orekit.utils.DoubleArrayDictionary) -> 'SpacecraftState':
        """
            Create a new instance with input additional data.
        
            Parameters:
                additionalStateDerivatives (:class:`~org.orekit.utils.DoubleArrayDictionary`): additional state derivatives
        ...
        """
        ...
    def withAttitude(self, attitude: org.orekit.attitudes.Attitude) -> 'SpacecraftState':
        """
            Create a new instance with input attitude.
        
            Parameters:
                newAttitude (:class:`~org.orekit.attitudes.Attitude`): attitude
        ...
        """
        ...
    def withMass(self, double: float) -> 'SpacecraftState':
        """
...
class AbstractMatricesHarvester(MatricesHarvester):
    ...
    def freezeColumnsNames(self) -> None:
        ...
        """
        ...
    def getInitialJacobianColumn(self, string: str) -> typing.MutableSequence[float]:
        """
...
class AbstractPropagator(Propagator):
    ...
    public abstract class AbstractPropagator extends :class:`~org.orekit.propagation.https:.docs.oracle.com.javase.8.docs.api.java.lang.Object?is` implements :class:`~org.orekit.propagation.Propagator`
        ...
        Common handling of :class:`~org.orekit.propagation.Propagator` methods for propagators.
    ...
    def setupMatricesComputation(self, string: str, realMatrix: org.hipparchus.linear.RealMatrix, doubleArrayDictionary: org.orekit.utils.DoubleArrayDictionary) -> MatricesHarvester:
        ...
        """
        ...
    def updateAdditionalData(self, spacecraftState: SpacecraftState) -> SpacecraftState:
        """
...
class AbstractStateModifier(AdditionalDataProvider[typing.MutableSequence[float]]):
    """
    public abstract class AbstractStateModifier extends :class:`~org.orekit.propagation.https:.docs.oracle.com.javase.8.docs.api.java.lang.Object?is` implements :class:`~org.orekit.propagation.AdditionalDataProvider`<double[]>
    
        Abstract base class for modifying state during propagation.
    
        This class is a specialized implementation of :class:`~org.orekit.propagation.AdditionalDataProvider` with a name set to
        the empty string and returning a null additional state.
        ...
        Also see:
            :class:`~org.orekit.propagation.Propagator`, :class:`~org.orekit.propagation.AdditionalDataProvider`
    """
    def __init__(self): ...
    def change(self, spacecraftState: SpacecraftState) -> SpacecraftState:
        ...
        """
        ...
    def getAdditionalData(self, spacecraftState: SpacecraftState) -> typing.MutableSequence[float]:
        """
            Get the additional data.
    ...
    def update(self, spacecraftState: SpacecraftState) -> SpacecraftState:
        """
            ...
            Specified by:
                :meth:`~org.orekit.propagation.AdditionalDataProvider.update` in
                interface :class:`~org.orekit.propagation.AdditionalDataProvider`
...
class FieldAbstractPropagator(FieldPropagator[_FieldAbstractPropagator__T], typing.Generic[_FieldAbstractPropagator__T]):
    ...
    public abstract class FieldAbstractPropagator<T extends :class:`~org.orekit.propagation.https:.www.hipparchus.org.apidocs.org.hipparchus.CalculusFieldElement?is`<T>> extends :class:`~org.orekit.propagation.https:.docs.oracle.com.javase.8.docs.api.java.lang.Object?is` implements :class:`~org.orekit.propagation.FieldPropagator`<T>
        ...
        including all propagation modes support and discrete events support for any simple propagation method.
    """
    def addAdditionalDataProvider(self, fieldAdditionalDataProvider: FieldAdditionalDataProvider[typing.Any, _FieldAbstractPropagator__T]) -> None: ...
    ...
    def setAttitudeProvider(self, attitudeProvider: org.orekit.attitudes.AttitudeProvider) -> None:
        ...
        """
        ...
    def updateAdditionalData(self, fieldSpacecraftState: FieldSpacecraftState[_FieldAbstractPropagator__T]) -> FieldSpacecraftState[_FieldAbstractPropagator__T]: ...
...
class FieldAbstractStateModifier(FieldAdditionalDataProvider[typing.MutableSequence[_FieldAbstractStateModifier__T], _FieldAbstractStateModifier__T], typing.Generic[_FieldAbstractStateModifier__T]):
    ...
    public abstract class FieldAbstractStateModifier<T extends :class:`~org.orekit.propagation.https:.www.hipparchus.org.apidocs.org.hipparchus.CalculusFieldElement?is`<T>> extends :class:`~org.orekit.propagation.https:.docs.oracle.com.javase.8.docs.api.java.lang.Object?is` implements :class:`~org.orekit.propagation.FieldAdditionalDataProvider`<T[], T>
        ...
        This class is a specialized implementation of :class:`~org.orekit.propagation.FieldAdditionalDataProvider` with a name
        set to the empty string and returning a null additional data.
        ...
        Also see:
            :class:`~org.orekit.propagation.Propagator`, :class:`~org.orekit.propagation.FieldAdditionalDataProvider`
    """
    def __init__(self): ...
    ...
    def getAdditionalData(self, fieldSpacecraftState: FieldSpacecraftState[_FieldAbstractStateModifier__T]) -> typing.MutableSequence[_FieldAbstractStateModifier__T]: ...
    def getName(self) -> str:
        """
            Get the name of the additional data.
...
class StateCovarianceMatrixProvider(AdditionalDataProvider[org.hipparchus.linear.RealMatrix]):
    """
    public class StateCovarianceMatrixProvider extends :class:`~org.orekit.propagation.https:.docs.oracle.com.javase.8.docs.api.java.lang.Object?is` implements :class:`~org.orekit.propagation.AdditionalDataProvider`<:class:`~org.orekit.propagation.https:.www.hipparchus.org.apidocs.org.hipparchus.linear.RealMatrix?is`>
    ...
    def getAdditionalData(self, spacecraftState: SpacecraftState) -> org.hipparchus.linear.RealMatrix:
        """
            Get the additional data.
...
class ToleranceProvider(CartesianToleranceProvider):
    """
    public interface ToleranceProvider extends :class:`~org.orekit.propagation.CartesianToleranceProvider`
    
        Interface to define integration tolerances for adaptive schemes (like the embedded Runge-Kutta ones) propagating the
        position-velocity vector (or an equivalent set of coordinates) and the mass, for a total of 7 primary dependent
        variables (in that order). The tolerances are given as an array of array: each row has 7 elements, whilst the first
        column is the absolute tolerances and the second the relative ones.
    ...
    """
    @staticmethod
    def getDefaultToleranceProvider(double: float) -> 'ToleranceProvider':
        """
            Defines a default tolerance provider. It is consistent with values from previous versions of Orekit obtained via other
            APIs.
    ...
    def getTolerances(self, fieldVector3D: org.hipparchus.geometry.euclidean.threed.FieldVector3D[_getTolerances_2__T], fieldVector3D2: org.hipparchus.geometry.euclidean.threed.FieldVector3D[_getTolerances_2__T]) -> typing.MutableSequence[typing.MutableSequence[float]]:
        """
            ...
            Parameters:
                referenceOrbit (:class:`~org.orekit.orbits.FieldOrbit`<T> referenceOrbit): orbit
                propagationOrbitType (:class:`~org.orekit.orbits.OrbitType`): orbit type for propagation (can be different from the input orbit one)
                positionAngleType (:class:`~org.orekit.orbits.PositionAngleType`): reference position angle type
    ...
    @typing.overload
    @staticmethod
    def of(double: float, double2: float) -> 'ToleranceProvider':
        """
    ...
    def of(cartesianToleranceProvider: typing.Union[CartesianToleranceProvider, typing.Callable]) -> 'ToleranceProvider':
        """
            ...
            Parameters:
                cartesianToleranceProvider (:class:`~org.orekit.propagation.CartesianToleranceProvider`): tolerance provider dedicated to Cartesian propagation
