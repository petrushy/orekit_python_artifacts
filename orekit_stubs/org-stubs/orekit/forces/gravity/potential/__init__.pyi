...
class ICGEMFormatReader(PotentialCoefficientsReader):
    ...
    public class ICGEMFormatReader extends :class:`~org.orekit.forces.gravity.potential.PotentialCoefficientsReader`
        ...
        (starting with eigen-6 model). Both versions are supported by the class.
        ...
        In order to simplify implementation, some design choices have been made: the reference date and the periods of harmonic
        pulsations are stored globally and not on a per-coefficient basis. This has the following implications:
          ...
          - only one occurrence of each coefficient may appear in the file, otherwise an error will be triggered during parsing.
            Multiple occurrences with different time stamps are forbidden (both because they correspond to a duplicated entry and
            because they define two different reference dates as per previous design choice).
